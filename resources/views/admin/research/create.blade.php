...
@section('content')
    ...
    <div class="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden max-w-4xl">
        ...
        <form action="{{ route('admin.research.store') }}" method="POST" enctype="multipart/form-data" class="p-8 sm:p-10">
            @csrf <h3 class="text-xl font-bold text-slate-800 mb-6 border-b border-slate-100 pb-4">Informasi Umum</h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div>
                    ...
                    <input type="text" name="leader_name" id="leader_name" value="{{ old('leader_name') }}" required
                           class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-[#800000]/20 focus:border-[#800000] outline-none transition @error('leader_name') border-red-500 @enderror"
                           placeholder="Contoh: Dr. Budi Santoso">
                    @error('leader_name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="status" id="status" required class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-[#800000]/20 focus:border-[#800000] outline-none transition bg-white @error('status') border-red-500 @enderror">
                ...
                <div class="flex items-center justify-center w-full">
                    <label class="flex flex-col items-center justify-center w-full h-32 border-2 border-slate-300 border-dashed rounded-xl cursor-pointer bg-slate-50 hover:bg-slate-100 transition" id="drop-area">
                        <div class="flex flex-col items-center justify-center pt-5 pb-6">
                            ...
                            <p class="text-xs text-slate-400 mt-1 transition-opacity duration-300" id="file-hint">PNG, JPG, JPEG, WEBP (Maks. 2MB)</p>
                        </div>
    ...
    <script>
        function showFileName(input) {
            ...
            if (input.files && input.files[0]) {
                ...
                fileHint.classList.add('hidden'); // Sembunyikan panduan ukuran

                // Ubah gaya area drop agar terlihat sukses
