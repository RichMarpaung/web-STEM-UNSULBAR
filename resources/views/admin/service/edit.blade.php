...
@section('content')
    ...
    <div class="max-w-3xl mx-auto">
        <div class="mb-8 flex items-center justify-between">
            <a href="{{ route('admin.s  ervice.index') }}"
            ...
            <form action="{{ route('admin.service.update', $communityService->slug) }}" method="POST" enctype="multipart/form-data" class="p-8 sm:p-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    ...
                    <div>
                        ...
                        <input type="date" name="date" id="date" value="{{ old('date', $communityService->date ? \Carbon\Carbon::parse($communityService->date)->format('Y-m-d') : '') }}" required
                        ...
                        @foreach($teams as $team)
                            @php
                                $isSelected = (is_array(old('team_ids')) && in_array($team->id, old('team_ids'))) ||
                                              (old('team_ids') === null && $communityService->teams->contains($team->id));
                            @endphp
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description" id="description" rows="5" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-[#800000]/20 focus:border-[#800000] outline-none transition resize-none">{{ old('description', $communityService->description) }}</textarea>
                </div>
                ...
                <div class="flex justify-end pt-6 border-t border-slate-100">
                    <button type="submit" class="px-8 py-3 bg-[#800000] text-white font-bold rounded-full shadow-lg shadow-red-900/30 hover:bg-[#6a0000] transition duration-200 flex items-center">
                        <i class="fa-solid fa-save mr-2"></i> Perbarui Agenda
                    </button>
                </div>
            </form>
        </div>
    ...
    <script>
        $(document).ready(function() {
            $('#team_ids').select2({ placeholder: "Ketik nama anggota pelaksana...", allowClear: true, width: '100%' });
        });
