...
@section('title', 'Tambah Anggota Tim | Admin STEM')
...
@section('content')
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden">
            <form action="{{ route('admin.team.store') }}" method="POST" id="teamForm" class="p-8 sm:p-10">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                    ...
                    <div>
                        ...
                        <input type="text" name="role" id="role" value="{{ old('role') }}" required
                            class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-[#800000]/20 focus:border-[#800000] outline-none transition"
                            placeholder="Contoh: Peneliti Utama">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                    <div>
                        ...
                        <input type="url" name="scopus_url" id="scopus_url" value="{{ old('scopus_url') }}"
                            class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-[#800000]/20 focus:border-[#800000] outline-none transition"
                            placeholder="https://www.scopus.com/authid/detail.uri?authorId=...">
                        @error('scopus_url') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="url" name="sinta_url" id="sinta_url" value="{{ old('sinta_url') }}"
                            class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-[#800000]/20 focus:border-[#800000] outline-none transition"
                            placeholder="https://sinta.kemdikbud.go.id/authors/profile/...">
                        @error('sinta_url') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="url" name="scholar_url" id="scholar_url" value="{{ old('scholar_url') }}"
                            class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-[#800000]/20 focus:border-[#800000] outline-none transition"
                            placeholder="https://scholar.google.co.id/citations?user=...">
                        @error('scholar_url') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
                </div>

                <div class="mb-10">
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Unggah Foto Profil (Rasio 1:1)</label>
                    <div class="flex items-center justify-center w-full relative">
                        <label class="flex flex-col items-center justify-center w-full h-48 border-2 border-slate-300 border-dashed rounded-xl cursor-pointer bg-slate-50 hover:bg-slate-100 transition overflow-hidden relative" id="drop-area">
                            ...
                            <div class="flex flex-col items-center justify-center pt-5 pb-6 z-10" id="upload-instruction">
                                ...
                                <p class="text-sm text-slate-500 text-center px-4">
                                    <span class="font-semibold text-[#800000]">Klik untuk mengunggah</span> dan memotong foto
                                </p>
                            </div>
                            <input type="file" id="image_input" class="hidden" accept="image/*" />
                        </label>
                    </div>
                    @error('cropped_image') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
    ...
    <div id="cropperModal" class="fixed inset-0 z-[99] hidden flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col">
            <div class="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                <h3 class="font-bold text-slate-800">Potong Foto Profil</h3>
            ...
            <div class="p-4 border-t border-slate-100 flex justify-end gap-3 bg-white">
                ...
                <button type="button" id="cropBtn" class="px-6 py-2.5 rounded-full text-sm font-bold text-white bg-emerald-500 hover:bg-emerald-600 shadow-md shadow-emerald-500/30 transition">Potong & Simpan</button>
            </div>
        </div>
    </div>
            ...
            cropBtn.addEventListener('click', function() {
                if (cropper) {
                    const canvas = cropper.getCroppedCanvas({
                        width: 600, // Ukuran simpan ideal HD
                        height: 600,
                    });
